...
class MultiPlotFigure:
    """Builder for a horizontal sequence of subplots.

    The class accumulates subplot registrations and then composes a Plotly
    figure with proper axis titles and annotations. Each registration is a
    simple dict describing the kind of subplot and its data.
    """

    def __init__(self, shared_yaxes: bool = True, title: str = "Time Series Overview") -> None:
        self.title = title
    ...
    def build_figure(self) -> go.Figure:
        ...
        fig = make_subplots(rows=1, cols=n_cols, shared_yaxes=self.shared_yaxes, subplot_titles=titles, specs=specs)
        ...
        for idx, reg in enumerate(self._registered, start=1):
            if reg["kind"] == "metric":
                ...
                if ptype == "area":
                    trace = go.Scatter(x=y, y=x, mode="lines", name=reg["title"], fill="tozeroy")

                elif ptype == "step":
                    trace = go.Scatter(x=df["State"], y=x, mode="lines+markers", name=reg["title"], line=dict(shape="vh", width=2))
                    fig.update_xaxes(categoryorder="array", categoryarray=list(df["State"].unique()), row=1, col=idx)
